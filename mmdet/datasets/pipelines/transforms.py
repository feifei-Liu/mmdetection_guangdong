...
@PIPELINES.register_module
class MinMaxIoURandomCrop(object):
    """Random crop the image & bboxes, the cropped patches have minimum IoU
    requirement with original image & bboxes, the IoU threshold is randomly
    selected from min_ious.
    ...
    def __init__(self, min_ious=(0.1, 0.3, 0.5, 0.7, 0.9), min_crop_size=0.3, max_crop_size=1,
                      sm_min_ious=(0.1, 0.3, 0.5, 0.7, 0.9), sm_min_crop_size=0.3, sm_max_crop_size=1,
                 ):
        # 1: return ori img
        self.sample_mode = min_ious
        self.min_crop_size = min_crop_size
        self.max_crop_size = max_crop_size

        self.sm_min_ious = sm_min_ious
        self.sm_min_crop_size = sm_min_crop_size
        self.sm_max_crop_size = sm_max_crop_size
    ...
    def __call__(self, results):
        ...
        while True:
            ...
            min_iou = mode
            for i in range(50):
                new_w = random.uniform(self.min_crop_size * w, self.max_crop_size * w)
                new_h = random.uniform(self.min_crop_size * h, self.max_crop_size * h)
                # h / w in [0.5, 2]
                if new_h / new_w < 0.5 or new_h / new_w > 2:
                    continue
